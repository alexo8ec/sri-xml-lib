...
class ObtenerDatos
{
    ...
    {
        //print_r($this->data['extensions']['subjectAltName']);exit;
        if (isset($this->data['extensions']['subjectAltName'])) {
            return $this->data['extensions']['subjectAltName'];
        }
    ...
    public function signXML($xml, $reference = '', $tag = null, $xmlns_xsi = false)
    {
        ...
        //$xml->save($carpeta);
        $comprobante = $doc->saveXML();
        $comprobante = $this->procesarRespuestaXml($comprobante);
        //header('Content-type: text/xml');
        //echo '<pre>';print_r($comprobante['comprobante']);exit;
        $hostname = 'mysql1005.mochahost.com';
        $database = 'sitagoa_facturalgo';
        $username = 'sitagoa_factural';
        $password = '0921605895aA';
        $mysqli = new mysqli($hostname, $username, $password, $database);
        if ($mysqli->connect_errno) {
            die("Fallo la conexión a MySQL: (" . $mysqli->mysqli_connect_errno() . ") " . $mysqli->mysqli_connect_error());
        } else {
            $sql = "UPDATE bm_estado_archivos SET archivo_firmado = '$comprobante[comprobante]' WHERE id_empresa = '14' AND claveAcceso='2305202001099030444000120010010000178580001785815';";
            $mysqli->query($sql);
        }
        return $comprobante;
    }
    ...
    function procesarRespuestaXml($xmlGenerado)
    {
        ...
        //if($isValido) {
        $comprobanteRespuesta = (string)$compelXml;
